...
class Tarefa:
    """
        ...
        `Database`.
    """
    def __init__(self: Self, titulo_tarefa: Optional[str], data_conclusao: 
                Optional[str] = None, id_tarefa: Optional[int] = None) -> None:
    ...
    def obter_tarefas(cls):
        with Database() as db:
            ...
            tarefas: list[Self] = [cls(titulo, data, id) 
                                   for titulo, data, id in resultados]
            return tarefas
    ...
    def alterar_status(self) -> Cursor:
        ...
        with Database() as db:
            query_verificar = """
            SELECT data_conclusao FROM tarefas WHERE id = ?;
            """
